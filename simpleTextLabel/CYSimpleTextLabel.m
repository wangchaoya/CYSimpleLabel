...
#define kCYTextLinkColor [UIColor colorWithRed:0/255.0 green:104/255.0 blue:248/255.0 alpha:1]


@interface CYLabelGestrueRecognizer : UIGestureRecognizer

- (void)swallowTouch;

@end

@interface CYLabelRichTextData ()

@property (nonatomic, strong) NSMutableArray<NSValue *> *rects;

@end

@implementation CYLabelRichTextData
...
@interface CYSimpleTextLabel () <UIGestureRecognizerDelegate>
@property (nonatomic, strong) UIColor *selectedBackgroundColor; //用户选中时候的背景颜色
@property (nonatomic, strong) CYLabelRichTextData *data;

@property (nonatomic, strong) NSMutableArray<CYLabelRichTextData *> *richTextDatas;

@property (nonatomic, strong) UITapGestureRecognizer *tapGestureRecognizer;

@property (nonatomic, strong) CYLabelGestrueRecognizer *labelGestureRecognizer;
...
- (instancetype)init{
    if (self = [super init]) {
        ...
        _labelGestureRecognizer = [[CYLabelGestrueRecognizer alloc] initWithTarget:self action:@selector(labelGestureHandler:)];
    ...
    for (NSTextCheckingResult *match in matches) {
        ...
        if ([match resultType] == NSTextCheckingTypeLink) {
            ...
            if ([url.scheme isEqualToString:URL_MAIL_SCHEME] ||
                ...
                CYLabelRichTextData *data = [[CYLabelRichTextData alloc] initWithType:CYLabelRichTextTypeURL];
            ...
            CYLabelRichTextData *data = [[CYLabelRichTextData alloc] initWithType:CYLabelRichTextTypePhoneNumber];
            data.range = matchRange;
...
- (CYLabelRichTextData *)richTextDataAtPoint:(CGPoint)point {
    CGFloat fraction;
    ...
    CYLabelRichTextData *data = cache[@(glyphIndex)];
    if (data) {
        return data;
    }
    ...
    for (CYLabelRichTextData *data in self.richTextDatas) {
...
- (void)_setNeedsDisplay:(CYLabelRichTextData *)data {
    self.data = data;
...
- (NSMutableAttributedString *)highlightDefaultDataTypes:(NSMutableAttributedString *)attributedString {
    ...
    for (NSTextCheckingResult *match in matches) {
        ...
        if (shouldHighlight) {
            [attributedString addAttribute:NSForegroundColorAttributeName value:kCYTextLinkColor range:matchRange];
        }
...
- (NSMutableAttributedString *)createAttributedStringWithEmotionString:(NSString *)emotionString font:(UIFont *)font lineSpacing:(NSInteger)lineSpacing {
    ...
    [[CYEmotionModelManager sharedManager]
    ...
    //高亮链接
    attributedString = [self highlightDefaultDataTypes:attributedString];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    if (gestureRecognizer == _tapGestureRecognizer) {
        CYLabelRichTextData *data = [self richTextDataAtPoint:point];
        if (data) {
            if (self.data) {
                [self _setNeedsDisplay:nil];
            }
...
- (void)labelGestureHandler:(CYLabelGestrueRecognizer *)labelGesture {
...
////////////////////////////////////////////////////
@interface CYLabelGestrueRecognizer ()

@property (nonatomic) UITouch *touch;

@end

@implementation CYLabelGestrueRecognizer
